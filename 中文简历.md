...
### 我的个人项目
- Yu（通用区块链框架）   
 https://github.com/Lawliet-Chan/yu  
 纯golang实现的通用区块链框架，从2021年1月开始到现在，仍然持续开发迭代中，使用该框架可实现：  
...
- YithQ（分布式消息队列）  
    https://github.com/Lawliet-Chan/yithQ  
    类似golang版的Kafka. 采用分区的方式提升topic的吞吐量. 内部有个名为zero的子系统功能类似etcd，它根据消息的权重来分配
    topic-partition。 我单独设计了一个存储引擎为了让他可以在FIFO场景下读写速度更快。尚未投入过生产使用，为学习所用，
    目前已经停止维护。
   
    
- qstore（基于磁盘的高性能队列）  
    https://github.com/Lawliet-Chan/qstore  
    在FIFO场景下，比正常策略下的LSM树读写更快，因为可以直接顺序写入进磁盘且摆脱不必要的LSM  log compaction，并且读取数据的时间复杂度为O（1）。
     可以用来做分布式消息系统的底层存储或者预写式日志。  
